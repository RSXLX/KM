...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const resp = await fetch(`${backend}/api/v1/admin/users/${params.id}`, { headers: authHeader ? { authorization: authHeader } : undefined });
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const tokenCookie = req.cookies.get('admin_token')?.value;
  const authHeader = auth || (tokenCookie ? `Bearer ${tokenCookie}` : undefined);
  const resp = await fetch(`${backend}/api/v1/admin/users/${params.id}/status`, {
    method: 'PUT',
